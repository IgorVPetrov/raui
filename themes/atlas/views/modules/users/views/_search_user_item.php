            ...
            if (issetModule('tariffPlans') && issetModule('paidservices') && ($data->id != Yii::app()->user->id)) {
                if (Yii::app()->user->isGuest) {
                    ...
                    if (!$defaultTariffInfo['showPhones']) {
                        //echo '<span>'. Yii::t('module_tariffPlans', 'Please <a href="{n}">login</a> to view', Yii::app()->controller->createUrl('/site/login')).'</span>';
                    }
                    else {
                        echo '<span><span id="owner-phone">' . CHtml::link(tc('Show phone'), 'javascript: void(0);', array('onclick' => 'getPhoneNum(this, '.$data->id.');')) . '</span>' . '</span>';
                    }
                }
                else {
                    if (TariffPlans::checkAllowShowPhone())
                        ...
                        //echo '<span>'.Yii::t('module_tariffPlans', 'Please <a href="{n}">change the tariff plan</a> to view', Yii::app()->controller->createUrl('/tariffPlans/main/index')).'</span>';
                    }
                }
            }
            else {
                echo '<li><span id="owner-phone">' . CHtml::link(tc('Show phone'), 'javascript: void(0);', array('onclick' => 'getPhoneNum(this, '.$data->id.');')) . '</span>' . '</li>';
            }
        }
		?>
		<?php echo '<span>' . $data->getLinkToAllListings() . '</span>'; ?>
	</p>
