...
class User extends ParentModel {
    ...
    public function getKvAd() {

        $sql = "SELECT count(id) FROM {{apartment}} WHERE active = 1 AND owner_active = 1 AND obj_type_id = 1 AND type = 2 AND (owner_id = " . $this->id . " )";
        return Yii::app()->db->createCommand($sql)->queryScalar();
    }

    public function getDmAd() {

        $sql = "SELECT count(id) FROM {{apartment}} WHERE active = 1 AND owner_active = 1 AND obj_type_id = 2 AND type = 2 AND (owner_id = " . $this->id . " )";
        return Yii::app()->db->createCommand($sql)->queryScalar();
    }
    ...
    public function rules() {
        return array(
            ...
            array('username, email', 'required', 'on' => 'register, register_without_confirm'),
...
//		}
    ...
    public function getAlluser($type = 'all', $limit = 'all') {
        ...
        $dependency = new CDbCacheDependency('SELECT MAX(date_updated) FROM {{user}}');
