...
//        }
    ...
    public static function getImgForSlider(){
		...
		if($images){
			foreach($images as $image){
				...
				$imgs[$image['id_object']]['src'] = $class = Yii::app()->theme->name != 'classic' ? Images::getThumbUrl($image, 663, 380) : Images::getThumbUrl($image, param('slider_img_width', 500), param('slider_img_height', 280));
            }
        }
	...
	public static function getListForEdit() {
		return CHtml::listData(PaidServices::model()->findAll('id!='.self::ID_ADD_FUNDS), 'id', 'name');
	}
