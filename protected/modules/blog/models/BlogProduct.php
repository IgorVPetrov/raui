...
/**
 * This is the model class for table "{{blog_product}}".
 *
 * The followings are the available columns in table '{{blog_product}}':
 * @property integer $id
 ...
 */
class BlogProduct extends CActiveRecord
{

    const RSS_PRODUCT_EN = '#';
	const RSS_PRODUCT_RU = '#';

	/**
	 * Returns the static model of the specified AR class.
	 * @param string $className active record class name.
	 * @return BlogProduct the static model class
	 */
    ...
    public static function getProductBlog(){
	    if (self::RSS_PRODUCT_EN) {
	        self::getFeedBlog(self::RSS_PRODUCT_EN);
	    }

	    if (self::RSS_PRODUCT_RU) {
		    self::getFeedBlog(self::RSS_PRODUCT_RU);
	    }
    }

	public static function getFeedBlog($rssUrl) {
		...
		if($xml){
			...
			foreach($xml->channel->item as $blog){
				...
				$sql = "SELECT id FROM {{blog_product}} WHERE link=:link";
				...
				$sql = "INSERT INTO {{blog_product}}
	                        (title, description, link, pubDate, author, lang)
	                        VALUES
	                        (:title, :description, :link, :pubDate, :author, :lang)
	                        ";

				$pubDate = self::_toMysqlDate((string) $blog->pubDate);

				$db->createCommand($sql)
						->bindValue(':title', (string) $blog->title, PDO::PARAM_STR)
						->bindValue(':description', (string) $blog->description, PDO::PARAM_STR)
						->bindValue(':link', $link, PDO::PARAM_STR)
						->bindValue(':pubDate', $pubDate, PDO::PARAM_STR)
						->bindValue(':author', '', PDO::PARAM_STR)
    ...
    public function getAllWithPagination($inCriteria = null){
		...
		$pages->pageSize = param('moduleBlog_blogPerPage', 10);
   		...
   		//$dependency = new CDbCacheDependency('SELECT MAX(pubDate) FROM {{blog_product}}');
    ...
    public static function getCountNoShow(){
        $sql = "SELECT COUNT(id) FROM {{blog_product}} WHERE is_show=0 AND lang=:lang";
