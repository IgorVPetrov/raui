...
class Apartment extends ParentModel {
	...
	public function rules() {
		...
		$rules = array(
			...
			array('square, land_square, ci_square', 'numerical'),
			...
			array('city_id, owner_active, active, type, obj_type_id, ownerEmail, ownerUsername, searchPaidService, deleted', 'safe', 'on' => 'search'),
    ...
    public function attributeLabels()
    {
        return array(
			...
			'ci_square' => tt('ci_square', 'apartments'),
	...
	public function getPrettyPrice($showPriceType = true) {
		...
		public function ObjgetPrettyPrice($showPriceType = true) {
			...
			if ($this->isPriceFromTo()) {
				...
				if ($priceTo) {
					$data = array(
						'{CURRENCY}' => $this->getCurrency(),
						'{TEXT_TO}' => tc('price_to'),
						'{PRICE}' => $this->BtsetPretty($priceTo),
					);
					$priceFromTo .= strtr($tpl['to'], $data);
				}
		...
		public function bt_getPrettyPrice($showPriceType = true) {
	...
	public function bt_getPriceMod($showPriceType = true) {
		...
		return $this->setPretty($price). ' ' . $this->getCurrency();
	...
	public function setPretty($price) {
		...
		public function BtsetPretty($price) {
	...
	public function getSquareString() {
		...
		if ($this->canShowInForm('ci_square')) {
			return $this->ci_square . " " . tc("ci_square");
		}
